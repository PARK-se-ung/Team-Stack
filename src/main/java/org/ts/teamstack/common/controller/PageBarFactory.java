...
public class PageBarFactory {
    ...
    public static StringBuffer dispatcherPageBuilder(PageInfo pageInfo, HttpServletRequest request, String url){
        int totalPage = (pageInfo.getTotalData() - 1) / pageInfo.getNumPerpage() + 1;
        int pageNo = 1, pageEnd = totalPage;
        if(pageInfo.getPageBarSize() < totalPage) {
            if(pageInfo.getPageBarSize() / 2 < pageInfo.getCurPage() && pageInfo.getCurPage() < totalPage - pageInfo.getPageBarSize() / 2) {
        ...
        if(pageInfo.getCurPage() == 1) {
            ...
            pageBar.append("<li class='page-item '>");
            pageBar.append("<a class='page-link' href=\"" + request.getContextPath() + url + "?cPage=" + (pageNo > 1? pageNo - 1 : 1) + "\"> prev </a>");
            pageBar.append("</li>");
        }
        for(int i = pageNo; i <= pageEnd; i++) {
            if(i == pageInfo.getCurPage()) {
                ...
                pageBar.append("<li class='page-item' data-no='" + i + "'>");
                pageBar.append("<a class='page-link' href=\"" + request.getContextPath() + url + "?cPage=" + i + "\"> " + i + " </a>");
            }
            pageBar.append("</li>");
        }
        if(pageInfo.getCurPage() == totalPage) {
            ...
            pageBar.append("<li class='page-item'>");
            pageBar.append("<a class='page-link' href=\"" + request.getContextPath() + url + "?cPage=" + (pageEnd < totalPage? pageEnd + 1 : totalPage) + "\"> next </a>");
        }
        pageBar.append("</li>");
        pageBar.append("</ul>");
